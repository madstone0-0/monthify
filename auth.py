...
logging.basicConfig(filename="monthly_playlist.log", encoding="utf-8", level=logging.INFO)
...
class Spotify:
    ...
    def get_user_saved_tracks(self):
        ...
        for i in range(0, MAX_RESULTS, 50):
            try:
                ...
                logger.error("Failed to reach spotify server trying", max_retries=MAX_TRIES)
        ...
        for month, year in self.playlist_names:
            for idx, item in enumerate(playlists):
                if (month + " '" + year[2:]).encode("utf-8").lower() == item[
                    ...
                    logger.info("Playlist name with ids", name=(month + " '" + year[2:]), id=item["id"])
    ...
    def sort_tracks_by_month(self):
        ...
        try:
            ...
            log.error("playlist_names and playlist_names_with_id are not the same length",
        ...
        for month, year, p_id in self.playlist_names_with_id:
            ...
            if not track_uris:
                ...
                logger.info("Adding tracks to playlist", tracks=track_uris, playlist=p_id)
